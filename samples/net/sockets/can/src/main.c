...
static const struct zcan_filter zfilter = {
    ...
    // .std_id = 0x1,
    .rtr_mask = 1,
    .id_mask = 0x0000,  // CAN_STD_ID_MASK
};
...
static void rx_task(int *can_fd, int *do_close_period,
  ...
  while (1) {
    ...
    printk("INFO: [%d] CAN msg: type 0x%x RTR 0x%x EID 0x%x DLC 0x%x\n", fd,
           msg.id_type, msg.rtr, msg.id, msg.dlc);
    printk("INFO: [%d] CAN data: ", fd);
    for (int i = 0; i < msg.dlc; ++i) printk("%x ", msg.data[i]);
    printk("\n");
...
int setup_tx_task1() {
  ...
  tx_tid1 =
      k_thread_create(&tx_data1, tx_stack1, K_THREAD_STACK_SIZEOF(tx_stack1),
                      (k_thread_entry_t)tx_task, INT_TO_POINTER(fd), NULL, NULL,
...
int setup_tx_task2() {
  ...
  tx_tid2 =
      k_thread_create(&tx_data2, tx_stack2, K_THREAD_STACK_SIZEOF(tx_stack2),
                      (k_thread_entry_t)tx_task, INT_TO_POINTER(fd), NULL, NULL,
                      PRIORITY, 0, K_NO_WAIT);
  if (!tx_tid2) {
    ret = -ENOENT;
...
int setup_rx_task1() {
  ...
  rx_tid1 = k_thread_create(
      ...
      INT_TO_POINTER(CLOSE_PERIOD), &filter, PRIORITY, 0, K_NO_WAIT);
  if (!rx_tid1) {
    ret = -ENOENT;
    errno = -ret;
